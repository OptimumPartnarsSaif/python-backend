...
# =======================================================
print("--- Exercise 7: Restaurant Order System ---")

class MenuItem:
    """A class to represent a single item on a restaurant menu."""
    ...
    def display(self):
        # Returns a formatted string with the menu item's details.
        return f"{self.name} ({self.category}): ${self.price:.2f}"

class Order:
    """A class to manage a customer's order."""
    def __init__(self):
        self.items = []
    ...
    def display_order(self):
        # Prints a formatted list of all items in the order and the total cost.
        print("\n--- Your Order ---")
        if not self.items:
...
pizza = MenuItem("Pepperoni Pizza", 15.50, "Main")
salad = MenuItem("Garden Salad", 8.00, "Appetizer")
soda = MenuItem("Cola", 2.00, "Drink")
...
my_order.add_item(salad)
my_order.add_item(soda)

# Display the order.
my_order.display_order()

# Remove an item from the order.
my_order.remove_item("Garden Salad")

# Display the updated order.
my_order.display_order()

# Try to remove an item that's not there.
my_order.remove_item("Fries")
